# Bootstrappable Andaluzcoin Core Builds

This directory contains the files necessary to perform bootstrappable Andaluzcoin
Core builds.
...
To build Andaluzcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Andaluzcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/andaluzcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/andaluzcoin-core/guix.sigs).
