...
Tracing (USDT) support for Andaluzcoin Core on Linux using. For more information on
USDT support in Andaluzcoin Core see the [USDT documentation].
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof andaluzcoind)
```
...
Andaluzcoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof andaluzcoind)
```
...
In a different terminal, starting Andaluzcoin Core in SigNet mode and with
re-indexing enabled.
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof andaluzcoind)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof andaluzcoind)
```
