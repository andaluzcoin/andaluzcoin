...
This guide describes how to build andaluzcoind, command-line utilities, and GUI on NetBSD.
...
It is not necessary to build wallet functionality to run andaluzcoind or the GUI.

###### Descriptor Wallet Support

`sqlite3` is required to enable support for [descriptor wallets](https://github.com/andaluzcoin/andaluzcoin/blob/master/doc/descriptors.md).
...
Andaluzcoin Core can provide notifications via ZeroMQ. If the package is installed, support will be compiled in.
```bash
pkgin zeromq
```
...
There are many ways to configure Andaluzcoin Core. Here is an example that
