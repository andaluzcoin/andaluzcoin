# CJDNS support in Andaluzcoin Core

It is possible to run Andaluzcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Andaluzcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Andaluzcoin network and individual nodes.
...
before it will work with your Andaluzcoin Core node. This is described in steps
...
## Run Andaluzcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Andaluzcoin Core
...
When enabled, this option tells Andaluzcoin Core that it is running in an
...
IPv6 local network. This helps Andaluzcoin Core perform better address management:
...
There are several ways to see your CJDNS address in Andaluzcoin Core:
