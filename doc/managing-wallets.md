...
The following command, for example, creates a descriptor wallet. More information about this command may be found by running `andaluzcoin-cli help createwallet`.
...
| -----------------|:------------------------------------------------------------|
| Linux            | `/home/<user>/.andaluzcoin/wallets`                             |
...
After encrypting the wallet or changing the passphrase, a new backup needs to be created immediately. The reason is that the keypool is flushed and a new HD seed is generated after encryption. Any andaluzcoins received by the new seed cannot be recovered from the previous backups.
...
If the wallet is encrypted and the user tries any operation related to private keys, such as sending andaluzcoins, an error message will be displayed.
...
In the GUI, there is no specific menu item to unlock the wallet. When the user sends andaluzcoins, the passphrase will be prompted automatically.
...
If both the wallet and all backups are lost for any reason, the andaluzcoins related to this wallet will become permanently inaccessible.
