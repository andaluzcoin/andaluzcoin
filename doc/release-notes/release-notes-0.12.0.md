Andaluzcoin Core version 0.12.0 is now available from:

  <https://andaluzcoin.org/bin/andaluzcoin-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of Andaluzcoin Core or other software:
...
pre-0.12 versions of Andaluzcoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Andaluzcoin Core version 0.11 or
earlier.
...
ECDSA signatures inside Andaluzcoin transactions now use validation using
[libsecp256k1](https://github.com/andaluzcoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/andaluzcoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Andaluzcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Andaluzcoin Core was to raise the default minimum
relay fee.

Andaluzcoin Core 0.12 will have a strict maximum size on the mempool. The
...
Andaluzcoin Core 0.12 also introduces new default policy limits on the length and
...
Andaluzcoin Core 0.12 nodes. Andaluzcoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/andaluzcoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Andaluzcoin Core 0.12 does not yet have support for
...
This allows running andaluzcoind without having to do any manual configuration.
...
Andaluzcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Andaluzcoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Andaluzcoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Andaluzcoin Core 0.13, and it is at this time undecided whether the
...
Andaluzcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Andaluzcoin Core automatically creates a hidden service to listen on, without
manual configuration. Andaluzcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Andaluzcoin Core is listening, and
...
Andaluzcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Andaluzcoin
Core to use floating fees. By default, Andaluzcoin Core will use floating
fees.
...
`andaluzcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/andaluzcoin/andaluzcoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/andaluzcoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/andaluzcoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /andaluzcoinrpc>
        ...
        # optional bypass andaluzcoind rpc basic auth
...
Restarting andaluzcoind will no longer clear out the list of banned peers; instead
...
- #6269 `d4565b6` gitian: Use the new andaluzcoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include andaluzcoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix andaluzcoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of andaluzcoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override ANDALUZCOIND and ANDALUZCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [andaluzcoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add ANDALUZCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libandaluzcoinconsensus (Braydon Fuller)
