...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Andaluzcoin Core will no longer
...
- The `startupnotify` option is used to specify a command to
  execute when Andaluzcoin Core has finished with its startup
  sequence. (#15367)
...
Users who already have a Andaluzcoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Andaluzcoin Core.
...
The change to using descriptors changes the default derivation paths used by Andaluzcoin Core
...
- #20198 Show name, format and if uses descriptors in andaluzcoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add andaluzcoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in andaluzcoin-cli (laanwj)
...
### Build system
- #18504 Drop andaluzcoin-tx and andaluzcoin-wallet dependencies on libevent (ryanofsky)
...
- #18653 add coverage for andaluzcoin-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Andaluzcoin Core using Honggfuzz (practicalswift)
...
- #18817 Document differences in andaluzcoind and andaluzcoin-qt locale handling (practicalswift)
