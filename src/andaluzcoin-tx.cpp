...
static void SetupAndaluzcoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " andaluzcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The andaluzcoin-tx tool is used for creating and modifying andaluzcoin transactions.\n\n"
                "andaluzcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded andaluzcoin transaction, or with \"-create [commands]\" to create a hex-encoded andaluzcoin transaction.\n"
                "\n"
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded andaluzcoin transaction
