// Copyright (c) 2016-present The Andaluzcoin Core developers
...
static std::optional<int> WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (missing_args || HelpRequested(args) || args.GetBoolArg("-version", false)) {
        std::string strUsage = strprintf("%s andaluzcoin-wallet utility version", CLIENT_NAME) + " " + FormatFullVersion() + "\n";
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                ...
                "By default andaluzcoin-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n\n"
                "To change the target wallet, use the -datadir, -wallet and (test)chain selection arguments.\n"
                "\n"
                "Usage: andaluzcoin-wallet [options] <command>\n"
                "\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
