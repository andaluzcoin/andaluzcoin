...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see andaluzcoind -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for andaluzcoind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect andaluzcoind signal handlers
    noui_connect();
