...
namespace init {
namespace {
const char* EXE_NAME = "andaluzcoin-node";

class AndaluzcoinNodeInit : public interfaces::Init
{
public:
    AndaluzcoinNodeInit(node::NodeContext& node, const char* arg0)
        : m_node(node),
...
std::unique_ptr<Init> MakeNodeInit(node::NodeContext& node, int argc, char* argv[], int& exit_status)
{
    ...
    // Check if andaluzcoin-node is being invoked as an IPC server. If so, then
