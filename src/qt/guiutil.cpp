...
#include <qt/andaluzcoinaddressvalidator.h>
...
{
    // return if URI is not valid or is no andaluzcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("andaluzcoin"))
        return false;
...
QString formatAndaluzcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("andaluzcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openAndaluzcoinConf()
{
    ...
    /* Open andaluzcoin.conf with the associated application */
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "andaluzcoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("andaluzcoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a andaluzcoin.desktop file to the autostart directory:
