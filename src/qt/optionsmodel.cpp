...
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // instead of strings, because andaluzcoin 22.x will not try to read these.
...
//! Convert enabled/size values to andaluzcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from andaluzcoin -prune setting.
...
//! Get pruning size value to show in GUI from andaluzcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (unit.canConvert<AndaluzcoinUnit>()) {
        ...
        m_display_andaluzcoin_unit = AndaluzcoinUnit::BTC;
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if andaluzcoin.conf value
