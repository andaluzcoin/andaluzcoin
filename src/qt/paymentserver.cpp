...
const QString BITCOIN_IPC_PREFIX("andaluzcoin:");
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click andaluzcoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start andaluzcoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling andaluzcoin: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // andaluzcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseAndaluzcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Andaluzcoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
