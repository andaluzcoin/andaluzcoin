...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_ibd_exit_after_loading_blocks, ChainTestingSetup)
{
    ...
    auto apply{[&](bool cached_is_ibd, bool loading_blocks, bool tip_exists, bool enough_work, bool tip_recent) {
        ...
        if (tip_exists) {
            ...
            const bool work_gate_applies{minimum_chain_work != arith_uint256{}};
            tip.nChainWork = minimum_chain_work - ((work_gate_applies && !enough_work) ? 1 : 0);
            ...
            for (const bool tip_exists : {false, true}) {
                for (const bool enough_work : {false, true}) {
                    for (const bool tip_recent : {false, true}) {
                        ...
                        // Andaluzcoin starts with nMinimumChainWork unset/zero.
                        // In that case there is no meaningful "insufficient work"
                        // state to test, because every valid chain has at least the
                        // configured minimum work.
                        const bool work_gate_applies{chainman.MinimumChainWork() != arith_uint256{}};
                        const bool expected_ibd = cached_is_ibd && (loading_blocks || !tip_exists || (work_gate_applies && !enough_work) || !tip_recent);
