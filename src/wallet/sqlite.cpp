...
static std::string SQLiteErrorString(sqlite3* db, int rc)
{
    if (db != nullptr) {
    ...
    return strprintf("SQLite error code %d", rc);
}

static std::string SQLiteErrorString(sqlite3_stmt* stmt, int rc)
{
    return SQLiteErrorString(stmt ? sqlite3_db_handle(stmt) : nullptr, rc);
}
...
{
    {
        ...
        if (++g_sqlite_count == 1) {
            ...
            if (ret != SQLITE_OK) {
                throw std::runtime_error(strprintf("SQLiteDatabase: Failed to setup error log: %s\n", SQLiteErrorString(static_cast<sqlite3*>(nullptr), ret)));
            }
...
bool SQLiteDatabase::Verify(bilingual_str& error)
{
    ...
    while (true) {
        ...
        if (ret != SQLITE_ROW) {
            error = strprintf(_("SQLiteDatabase: Failed to execute statement to verify database: %s"), SQLiteErrorString(stmt, ret));
            break;
        }
        const char* msg = (const char*)sqlite3_column_text(stmt, 0);
        if (!msg) {
            error = strprintf(_("SQLiteDatabase: Failed to read database verification error: %s"), SQLiteErrorString(stmt, ret));
            break;
        }
...
bool SQLiteDatabase::Backup(const std::string& dest) const
{
    ...
    if (res != SQLITE_DONE) {
        LogPrintf("%s: Unable to backup: %s\n", __func__, SQLiteErrorString(db_copy, res));
...
bool SQLiteBatch::ReadKey(DataStream&& key, DataStream& value)
{
    ...
    if (res != SQLITE_ROW) {
        if (res != SQLITE_DONE) {
            ...
            LogPrintf("%s: Unable to execute statement: %s\n", __func__, SQLiteErrorString(m_read_stmt, res));
        }
...
bool SQLiteBatch::WriteKey(DataStream&& key, DataStream&& value, bool overwrite)
{
    ...
    sqlite3_reset(stmt);
    if (res != SQLITE_DONE) {
        LogPrintf("%s: Unable to execute statement: %s\n", __func__, SQLiteErrorString(stmt, res));
    }
...
DatabaseCursor::Status SQLiteCursor::Next(DataStream& key, DataStream& value)
{
    ...
    if (res != SQLITE_ROW) {
        LogPrintf("%s: Unable to execute cursor step: %s\n", __func__, SQLiteErrorString(m_cursor_stmt, res));
        return Status::FAIL;
    }
...
SQLiteCursor::~SQLiteCursor()
{
    ...
    sqlite3* db = m_cursor_stmt ? sqlite3_db_handle(m_cursor_stmt) : nullptr;
