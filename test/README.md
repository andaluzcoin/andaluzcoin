This directory contains integration tests that test andaluzcoind and its
...
andaluzcoind and andaluzcoin-qt by interacting with them through the RPC and P2P
interfaces.
...
Before tests can be run locally, Andaluzcoin Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the andaluzcoind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another andaluzcoind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its andaluzcoind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other andaluzcoind processes are running.

On linux, the test framework will warn if there is another
andaluzcoind process running when the tests are started.

If there are zombie andaluzcoind processes after test failure, you can kill them
...
```bash
killall andaluzcoind
```

or

```bash
pkill -9 andaluzcoind
```
...
sure andaluzcoind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and andaluzcoind `debug.log`s will all be dumped
...
`test_framework.log` and andaluzcoind `debug.log`s can be combined into a single
...
call methods that interact with the andaluzcoind nodes-under-test.

If further introspection of the andaluzcoind instances themselves becomes
