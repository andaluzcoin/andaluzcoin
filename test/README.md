This directory contains integration tests that testandaluzcoind and its
...
The P2P and RPC ports used by theandaluzcoind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is anotherandaluzcoind
process running on the system (perhaps from a previous test which hasn't successfully
killed all itsandaluzcoind nodes), then there may be a port conflict which will
...
sureandaluzcoind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` andandaluzcoind `debug.log`s will all be dumped
...
`test_framework.log` andandaluzcoind `debug.log`s can be combined into a single
...
call methods that interact with theandaluzcoind nodes-under-test.

If further introspection of theandaluzcoind instances themselves becomes
