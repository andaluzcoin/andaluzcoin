...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the AndaluzcoinTestFramework, place overrides for the
...
- Use `platform.system()` for detecting the running operating system and `os.name` to
  ...
  methods in the `AndaluzcoinTestFramework` class, which can be used to skip a whole test
...
with the andaluzcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a andaluzcoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Andaluzcoin Core node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the AndaluzcoinTestFramework
...
Test objects for interacting with a andaluzcoind node over the p2p interface.
