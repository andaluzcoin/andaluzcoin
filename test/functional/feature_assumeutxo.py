#!/usr/bin/env python3
# Copyright (c) 2021-present The Andaluzcoin Core developers
...
to a hash that has been compiled into andaluzcoind.
...
class AssumeutxoTest(AndaluzcoinTestFramework):
    ...
    def test_invalid_snapshot_scenarios(self, valid_snapshot_path):
        ...
        for [magic, name, real] in invalid_magics:
            ...
            if real:
                # main/test → generic unrecognized; signet → explicit mismatch
                if name == "signet":
                    msg = f"Unable to parse metadata: The network of the snapshot ({name}) does not match the network of this node (regtest)."
                else:
                    msg = "Unable to parse metadata: This snapshot has been created for an unrecognized network"
                assert_raises_rpc_error(parsing_error_code, msg, node.loadtxoutset, bad_snapshot_path)
            else:
                # completely unknown magic → extended guidance
                assert_raises_rpc_error(
                    parsing_error_code,
                    ...
                    node.loadtxoutset, bad_snapshot_path
                )
    ...
    def test_headers_not_synced(self, valid_snapshot_path):
        for node in self.nodes[1:]:
            msg = "Unable to load UTXO snapshot: The base block header (006a6cb9d1d6a4effe714cb175d7b4bbf4788dadb46d2d1d6b613e6a9a890f96) must appear in the headers chain. Make sure all headers are syncing, and call loadtxoutset again."
    ...
    def test_invalid_mempool_state(self, dump_output_path):
        self.log.info("Test andaluzcoind should fail when mempool not empty.")
        node=self.nodes[2]
    ...
    def test_invalid_file_path(self):
        self.log.info("Test andaluzcoind should fail when file path is invalid.")
        node = self.nodes[0]
    ...
    def test_snapshot_with_less_work(self, dump_output_path):
        self.log.info("Test andaluzcoind should fail when snapshot has less accumulated work than this node.")
        node = self.nodes[0]
    ...
    def test_snapshot_block_invalidated(self, dump_output_path):
        ...
        for height in [SNAPSHOT_BASE_HEIGHT, SNAPSHOT_BASE_HEIGHT - 1]:
            ...
            msg = "Unable to load UTXO snapshot: The base block header (006a6cb9d1d6a4effe714cb175d7b4bbf4788dadb46d2d1d6b613e6a9a890f96) is part of an invalid chain."
            assert_raises_rpc_error(-32603, msg, node.loadtxoutset, dump_output_path)
    ...
    def run_test(self):
        ...
        # fixed. See also https://github.com/andaluzcoin/andaluzcoin/issues/26245
