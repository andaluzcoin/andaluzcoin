...
class PruneTest(AndaluzcoinTestFramework):

    # ---------- canonical path helpers ----------
    def _node_datadir(self, idx: int) -> str:
        """
        Return the filesystem datadir for node `idx`. Some forks accidentally wrap
        node.datadir with an RPC proxy; in that case, fall back to the standard
        tmpdir layout (<tmpdir>/node{idx}).
        """
    ...
    # ---- Path helpers (robust against accidental self.nodes rebinds) ----
    def _node_datadir(self, idx: int) -> str:
        # Prefer the live TestNode.datadir
        d = getattr(self.nodes[idx], "datadir", None)
        if isinstance(d, str):
            return d
        # Fallback to cached list set early in run_test()
        if hasattr(self, "node_datadirs"):
            return self.node_datadirs[idx]
        raise AssertionError(f"node {idx} has no datadir; self.nodes likely overwritten")

    def _node_blocks_dir(self, idx: int) -> str:
        datadir = self._node_datadir(idx)
        if not isinstance(datadir, str):
            raise AssertionError(f"_node_datadir({idx}) returned non-str: {type(datadir)} {datadir!r}")
        chain_dir = getattr(self, "_chain_dirname", "regtest")
    ...
    # --- blk*.dat helpers (used by pruning checks) ---
    ...
    def set_test_params(self):
        ...
        self.num_nodes = 6
        # Keep TestNode objects intact; store RPC proxies separately
        self._rpcs = None
