...
class MempoolLimitTest(AndaluzcoinTestFramework):
    ...
    def test_mid_package_eviction_success(self):
        ...
        fees_luz_per_kvb = [entry["fees"]["base"] / (Decimal(entry["vsize"]) / 1000) for entry in mempool_entries]
        mempool_entry_minrate = min(fees_luz_per_kvb)
        ...
        for eviction in eviction_set:
            ...
            for txid, entry in zip(mempool_txids, mempool_entries):
                if txid == eviction:
                    evicted_feerate_luz_per_kvb = entry["fees"]["modified"] / (Decimal(entry["vsize"]) / 1000)
                    assert_greater_than(evicted_feerate_luz_per_kvb, max_parent_feerate)
    ...
    def run_test(self):
        ...
        worst_feerate_luzvb = Decimal("21000000")
        for txid in current_mempool:
            entry = node.getmempoolentry(txid)
            worst_feerate_luzvb = min(worst_feerate_luzvb, entry["fees"]["descendant"] / entry["descendantsize"])
        ...
        child_feerate = (worst_feerate_luzvb * 1000) - Decimal("0.000001")  # 0.1 sats/vbyte below worst feerate
        ...
        assert_greater_than(worst_feerate_luzvb, package_fee / package_vsize)
