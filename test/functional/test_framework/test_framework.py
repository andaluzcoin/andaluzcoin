...
class AndaluzcoinTestFramework(metaclass=AndaluzcoinTestMetaClass):
    """Base class for a andaluzcoin test script.

    Individual andaluzcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave andaluzcoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop andaluzcoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use andaluzcoin-cli instead of RPC for all commands")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a andaluzcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple andaluzcoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the andaluzcoind
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as andaluzcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in andaluzcoin.conf
    ...
    def skip_if_no_andaluzcoind_tracepoints(self):
        """Skip the running test if andaluzcoind has not been compiled with USDT tracepoint support."""
    ...
    def skip_if_no_andaluzcoind_zmq(self):
        """Skip the running test if andaluzcoind has not been compiled with zmq support."""
    ...
    def is_cli_compiled(self):
        """Checks whether andaluzcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether andaluzcoin-wallet was compiled."""
