...
"""Class for andaluzcoind node under test"""
...
BITCOIN_PID_FILENAME_DEFAULT = "andaluzcoind.pid"
...
class TestNode():
    """A class for representing a andaluzcoind node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, andaluzcoind, andaluzcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        """
        Kwargs:
        ...
        # Configuration for logging is set as command-line args rather than in the andaluzcoin.conf file.
        # This means that starting a andaluzcoind using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
    ...
    def __del__(self):
        # Ensure that we don't leave any andaluzcoind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then andaluzcoind would bind P2P ports on
        ...
        # andaluzcoin.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
        ...
        # Add a new stdout and stderr file each time andaluzcoind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by andaluzcoind, and
    ...
    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the andaluzcoind process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; andaluzcoind is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to andaluzcoind after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; andaluzcoind is still starting
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into andaluzcoind")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to andaluzcoind
        expected_msg: regex that stderr should match when andaluzcoind fails

        Will throw if andaluzcoind starts without an error.
        Will throw if an expected_msg is provided and it does not match andaluzcoind's stdout."""
...
class TestNodeCLI():
    """Interface to andaluzcoin-cli for an individual node"""
    ...
    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with andaluzcoin-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run andaluzcoin-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running andaluzcoin-cli {}".format(p_args[2:]))
