...
"""Class forandaluzcoind node under test"""
...
class TestNode():
    """A class for representing aandaluzcoind node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor,andaluzcoind,andaluzcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        ...
        # Configuration for logging is set as command-line args rather than in theandaluzcoin.conf file.
        # This means that starting aandaluzcoind using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
    ...
    def __del__(self):
        # Ensure that we don't leave anyandaluzcoind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, thenandaluzcoind would bind P2P ports on
        ...
        # Add a new stdout and stderr file each timeandaluzcoind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway byandaluzcoind, and
    ...
    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to theandaluzcoind process. Returns False if unable to connect."""
        ...
        self._raise_assertion_error("Unable to connect toandaluzcoind after {}s".format(self.rpc_timeout))
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled intoandaluzcoind")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through toandaluzcoind
        expected_msg: regex that stderr should match whenandaluzcoind fails

        Will throw ifandaluzcoind starts without an error.
        Will throw if an expected_msg is provided and it does not matchandaluzcoind's stdout."""
...
class TestNodeCLI():
    """Interface toandaluzcoin-cli for an individual node"""
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Runandaluzcoin-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Runningandaluzcoin-cli {}".format(p_args[2:]))
