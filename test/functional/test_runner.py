...
`test/functional/test_framework/test_framework.py:AndaluzcoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_andaluzcoin_cli.py --legacy-wallet',
    'interface_andaluzcoin_cli.py --descriptors',
...
def main():
    ...
    print("DEBUG: Starting test runner with sys.stdout =", sys.stdout)
    sys.stdout.flush()
    ...
    print("Using test configuration file at:", configfile)
    ...
    logging.basicConfig(
            ...
            level=logging_level,  # Force DEBUG level
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if andaluzcoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "andaluzcoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a andaluzcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Andaluzcoin functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/andaluzcoin/andaluzcoin/pull/27561.
...
class RPCCoverage():
    ...
    commands per `andaluzcoin-cli help` (`rpc_interface.txt`).
