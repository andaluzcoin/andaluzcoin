...
"""Verify Andaluzcoin wallet fee estimation/configuration identity."""
...
class AndaluzWalletFeeEstimationIdentityTest(BitcoinTestFramework):
    ...
    def run_test(self):
        miner_wallet_name = "andaluz_fee_estimation_miner"
        sender_wallet_name = "andaluz_fee_estimation_sender"
        receiver_wallet_name = "andaluz_fee_estimation_receiver"

        amount = Decimal("1.00000000")
        coinbase_amount = Decimal("50.00000000")
        fallback_fee_rate_btc_kvb = Decimal("0.00010000")
        expected_fee_rate_sat_vb = Decimal("10.0")

        self.log.info("Checking initial Andaluzcoin runtime identity")
        self.assert_andaluz_runtime_identity()

        self.log.info("Checking fee estimation RPC is available on Andaluzcoin regtest")
        estimate_response = self.nodes[0].estimatesmartfee(6)
        assert "feerate" in estimate_response or "errors" in estimate_response
        assert "Bitcoin" not in str(estimate_response), estimate_response
        assert "Satoshi" not in str(estimate_response), estimate_response
        ...
        self.log.info("Using Andaluzcoin fallback fee configuration")
        assert_equal(fallback_fee_rate_btc_kvb, Decimal("0.00010000"))
        ...
        assert_equal(sender.getbalances()["mine"]["trusted"], coinbase_amount)

        self.log.info("Creating Andaluzcoin receiver address")
        receiver_address = receiver.getnewaddress(
            "andaluz-fee-estimation-receiver",
            "bech32",
        )
        self.assert_valid_wallet_address(receiver, receiver_address)

        self.log.info("Sending Andaluzcoin using configured fallback fee rate")
        ...
        assert actual_fee_rate_sat_vb >= expected_fee_rate_sat_vb, (
            actual_fee_rate_sat_vb,
            expected_fee_rate_sat_vb,
            decoded_tx,
            sender_unconfirmed_tx,
        )

        assert actual_fee_rate_sat_vb < expected_fee_rate_sat_vb + Decimal("1.0"), (
            actual_fee_rate_sat_vb,
            expected_fee_rate_sat_vb,
            decoded_tx,
            sender_unconfirmed_tx,
        ...
        self.log.info("Confirming configured-fee transaction")
        ...
        self.log.info("Checking confirmed Andaluzcoin wallet balances")
        self.wait_until(
...
if __name__ == "__main__":
    AndaluzWalletFeeEstimationIdentityTest(__file__).main()
