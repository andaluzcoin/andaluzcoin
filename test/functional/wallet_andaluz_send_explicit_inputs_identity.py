...
"""Verify Andaluzcoin wallet send explicit-input identity."""
...
class AndaluzWalletSendExplicitInputsIdentityTest(BitcoinTestFramework):
    ...
    def run_test(self):
        miner_wallet_name = "andaluz_send_inputs_miner"
        sender_wallet_name = "andaluz_send_inputs_sender"
        receiver_wallet_name = "andaluz_send_inputs_receiver"
        ...
        self.log.info("Mining two independent spendable ALUZ UTXOs to sender")
        ...
        self.log.info("Selecting one exact Andaluzcoin UTXO")
        ...
        self.log.info(
            "Creating signed send RPC transaction from explicit input"
        )
        ...
        self.log.info("Checking exact explicit input was selected")
        ...
        assert untouched_outpoint not in decoded_inputs, (
            untouched_outpoint,
            decoded_inputs,
        )
        ...
        self.log.info(
            "Checking explicit-input transaction was not broadcast automatically"
        )
        ...
        self.log.info(
            "Checking both source UTXOs remain unspent before broadcast"
        )

        prebroadcast_utxos = sender.listunspent(
            1,
            9999999,
        )
        ...
        assert self.has_utxo(
            prebroadcast_utxos,
            untouched_outpoint["txid"],
            untouched_outpoint["vout"],
        ), prebroadcast_utxos

        self.log.info(
            "Checking explicit-input transaction is acceptable to mempool"
        )
        ...
        self.log.info(
            "Broadcasting explicit-input Andaluzcoin transaction"
        )
        ...
        self.log.info(
            "Checking selected UTXO spent while untouched UTXO remains"
        )

        pending_sender_utxos = sender.listunspent(
            0,
            9999999,
        )

        assert not self.has_utxo(
            pending_sender_utxos,
            selected_outpoint["txid"],
            selected_outpoint["vout"],
        ), pending_sender_utxos
        ...
        self.log.info(
            "Checking sender recognizes explicit-input transaction"
        )
        ...
        self.log.info(
            "Checking receiver sees pending explicit-input payment"
        )
        ...
        self.log.info(
            "Confirming explicit-input Andaluzcoin transaction"
        )
        ...
        self.log.info(
            "Checking untouched Andaluzcoin UTXO still remains"
        )

        confirmed_sender_utxos = sender.listunspent(
            1,
            9999999,
        )

        assert not self.has_utxo(
            confirmed_sender_utxos,
            selected_outpoint["txid"],
            selected_outpoint["vout"],
        ), confirmed_sender_utxos
        ...
        self.log.info(
            "Checking confirmed explicit-input wallet history"
        )
...
if __name__ == "__main__":
    AndaluzWalletSendExplicitInputsIdentityTest(__file__).main()
