...
"""Verify Andaluzcoin wallet send locktime identity."""
...
class AndaluzWalletSendLocktimeIdentityTest(BitcoinTestFramework):
    ...
    def run_test(self):
        miner_wallet_name = "andaluz_send_locktime_miner"
        sender_wallet_name = "andaluz_send_locktime_sender"
        receiver_wallet_name = "andaluz_send_locktime_receiver"
        ...
        receiver_address = receiver.getnewaddress(
            "andaluz-send-locktime-receiver",
            "bech32",
        )
        ...
        requested_locktime = current_height + 1

        self.log.info(
            f"Creating locktime transaction for height {requested_locktime}"
        )
        ...
        self.log.info("Checking locktime transaction was added to sender wallet")

        assert self.wallet_has_txid(
            sender,
            txid,
        ), txid
        ...
        raw_hex = sender_pending_tx["hex"]
        assert raw_hex, sender_pending_tx
        ...
        self.log.info(
            "Checking locktime transaction was not accepted into mempool"
        )
        ...
        mempool_result = self.nodes[0].testmempoolaccept(
            [raw_hex],
        )
        ...
        assert_equal(
            mempool_result[0]["txid"],
            txid,
        )

        assert_equal(
            mempool_result[0]["allowed"],
            False,
        )

        assert_equal(
            mempool_result[0]["reject-reason"],
            "non-final",
        )

        self.log.info(
            "Checking receiver has not seen non-final transaction"
        )
        ...
        self.log.info(
            "Mining one block to satisfy Andaluzcoin transaction locktime"
        )

        unlock_block_hash = self.nodes[0].generatetoaddress(
            1,
            miner_mining_address,
            called_by_framework=True,
        )[0]
        ...
        assert_equal(
            sender.gettransaction(txid)["confirmations"],
            0,
        )

        self.log.info(
            "Checking transaction becomes mempool-valid after locktime"
        )

        unlocked_mempool_result = (
        ...
        assert_equal(
            len(unlocked_mempool_result),
            1,
        )

        assert_equal(
            unlocked_mempool_result[0]["txid"],
            txid,
        )

        assert_equal(
            unlocked_mempool_result[0]["allowed"],
            True,
        )

        self.log.info(
            "Broadcasting unlocked Andaluzcoin transaction"
        )
        ...
        self.log.info(
            "Checking receiver sees pending ALUZ after explicit broadcast"
        )
        ...
        self.log.info(
            "Confirming Andaluzcoin locktime transaction"
        )
        ...
        self.log.info(
            "Checking confirmed sender locktime accounting"
        )
        ...
        self.log.info(
            "Checking confirmed receiver locktime accounting"
        )
        ...
        self.log.info(
            "Checking confirmed locktime wallet history"
        )
...
if __name__ == "__main__":
    AndaluzWalletSendLocktimeIdentityTest(__file__).main()
