...
"""Verify Andaluzcoin wallet send non-broadcast identity."""
...
class AndaluzWalletSendNonBroadcastIdentityTest(BitcoinTestFramework):
    ...
    def run_test(self):
        miner_wallet_name = "andaluz_send_nonbroadcast_miner"
        sender_wallet_name = "andaluz_send_nonbroadcast_sender"
        receiver_wallet_name = "andaluz_send_nonbroadcast_receiver"
        ...
        receiver_address = receiver.getnewaddress(
            "andaluz-send-nonbroadcast-receiver",
            "bech32",
        )
        self.assert_valid_wallet_address(receiver, receiver_address)

        self.log.info("Creating signed transaction without broadcasting")
        ...
        self.log.info("Checking non-broadcast transaction identity")
        decoded_tx = self.nodes[0].decoderawtransaction(raw_hex)
        ...
        self.log.info("Checking transaction was not broadcast or added to wallet")
        assert_equal(
        ...
        self.log.info("Checking raw transaction is acceptable before broadcast")
        accept_result = self.nodes[0].testmempoolaccept([raw_hex])

        assert_equal(len(accept_result), 1)
        assert_equal(accept_result[0]["txid"], txid)
        assert_equal(accept_result[0]["allowed"], True)

        self.log.info("Broadcasting signed Andaluzcoin transaction explicitly")
        broadcast_txid = self.nodes[0].sendrawtransaction(raw_hex)
        ...
        self.log.info("Checking sender recognizes broadcast transaction")
        self.wait_until(
        ...
        self.log.info("Confirming explicitly broadcast Andaluzcoin transaction")
...
if __name__ == "__main__":
    AndaluzWalletSendNonBroadcastIdentityTest(__file__).main()
