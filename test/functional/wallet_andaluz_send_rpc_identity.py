...
"""Verify Andaluzcoin wallet send RPC identity."""
...
class AndaluzWalletSendRPCIdentityTest(BitcoinTestFramework):
    ...
    def run_test(self):
        miner_wallet_name = "andaluz_send_rpc_miner"
        sender_wallet_name = "andaluz_send_rpc_sender"
        receiver_wallet_name = "andaluz_send_rpc_receiver"
        ...
        receiver_address = receiver.getnewaddress(
            "andaluz-send-rpc-receiver",
            "bech32",
        )
        self.assert_valid_wallet_address(receiver, receiver_address)

        self.log.info("Sending ALUZ using modern wallet send RPC")
        ...
        self.log.info("Checking send RPC transaction was broadcast")
        ...
        self.log.info("Checking sender wallet transaction identity")
        sender_pending_tx = sender.gettransaction(
            txid=txid,
            verbose=True,
        )

        assert_equal(sender_pending_tx["amount"], -payment_amount)
        ...
        assert_equal(recipient_outputs[0]["value"], payment_amount)

        self.log.info("Checking transaction is visible in sender wallet history")
        sender_list_entry = self.find_wallet_tx(
            sender,
            txid,
            "send",
        )
        assert_equal(sender_list_entry["amount"], -payment_amount)
        assert_equal(sender_list_entry["confirmations"], 0)

        self.log.info("Checking receiver sees pending ALUZ")
        self.wait_until(
        ...
        assert_equal(receiver_list_entry["amount"], payment_amount)
        assert_equal(receiver_list_entry["confirmations"], 0)

        self.log.info("Confirming Andaluzcoin send RPC transaction")
        ...
        assert_equal(sender_confirmed_entry["amount"], -payment_amount)
        assert_equal(sender_confirmed_entry["confirmations"], 1)

        assert_equal(receiver_confirmed_entry["amount"], payment_amount)
        assert_equal(receiver_confirmed_entry["confirmations"], 1)
...
if __name__ == "__main__":
    AndaluzWalletSendRPCIdentityTest(__file__).main()
