...
"""Verify Andaluzcoin wallet subtract-fee-from-amount identity."""
...
class AndaluzWalletSubtractFeeFromAmountIdentityTest(BitcoinTestFramework):
    ...
    def run_test(self):
        miner_wallet_name = "andaluz_subtract_fee_miner"
        sender_wallet_name = "andaluz_subtract_fee_sender"
        receiver_wallet_name = "andaluz_subtract_fee_receiver"
        ...
        receiver_address = receiver.getnewaddress("andaluz-subtract-fee-receiver", "bech32")
        self.assert_valid_wallet_address(receiver, receiver_address)

        self.log.info("Sending Andaluzcoin with subtractfeefromamount enabled")
        ...
        assert actual_fee < gross_amount, sender_unconfirmed_tx

        expected_receiver_amount = gross_amount - actual_fee
        expected_sender_balance = initial_sender_trusted - gross_amount

        self.log.info("Checking receiver pending amount is gross amount minus fee")
        self.wait_until(
            lambda: receiver.getbalances()["mine"]["untrusted_pending"] == expected_receiver_amount,
            timeout=60,
        )

        receiver_pending_tx = receiver.gettransaction(txid)
        assert_equal(receiver_pending_tx["amount"], expected_receiver_amount)
        assert_equal(receiver_pending_tx["confirmations"], 0)

        self.log.info("Confirming subtract-fee-from-amount transaction")
        ...
        self.log.info("Checking receiver confirmed amount is gross amount minus fee")
        self.wait_until(
            lambda: receiver.getbalances()["mine"]["trusted"] == expected_receiver_amount,
            timeout=60,
        )

        receiver_tx = receiver.gettransaction(txid)
        assert_equal(receiver_tx["amount"], expected_receiver_amount)
        ...
        assert_equal(receiver_list_entry["amount"], expected_receiver_amount)
        ...
        assert_equal(sender_tx["amount"], -expected_receiver_amount)
...
if __name__ == "__main__":
    AndaluzWalletSubtractFeeFromAmountIdentityTest(__file__).main()
