...
"""Verify Andaluzcoin transaction fee identity."""
...
class AndaluzTransactionFeeIdentityTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Sending 1 ALUZ with wallet-selected transaction fee")
        txid = sender.sendtoaddress(receiver_addr, Decimal("1.00000000"))
        assert txid in self.nodes[0].getrawmempool()

        sender_tx = sender.gettransaction(txid)
        assert_equal(sender_tx["amount"], Decimal("-1.00000000"))
        assert "fee" in sender_tx, sender_tx
        assert sender_tx["fee"] < Decimal("0"), sender_tx

        fee_paid = -sender_tx["fee"]
        assert fee_paid > Decimal("0"), sender_tx

        self.log.info("Confirming Andaluzcoin transaction")
        ...
        self.log.info("Checking receiver amount and sender fee payment")
        assert_equal(receiver.getbalance(), Decimal("1.00000000"))

        # Mining the confirmation block also matures one additional 50 ALUZ
        # coinbase reward from the earlier 101-block mining run.
        expected_sender_balance = sender_balance_before - Decimal("1.00000000") - fee_paid + Decimal("50.00000000")
...
if __name__ == "__main__":
    AndaluzTransactionFeeIdentityTest(__file__).main()
