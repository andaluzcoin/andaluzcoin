...
class SimulateTxTest(AndaluzcoinTestFramework):
    ...
    def run_test(self):
       ...
       andaluzcoin_fee = Decimal(funding["fee"])

        # w0 sees fee + 5luz decrease, w2 sees + 5luz
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -(Decimal("5") +andaluzcoin_fee))
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -5 -andaluzcoin_fee + 4.9999       -5 -andaluzcoin_fee
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("5") -andaluzcoin_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("5") -andaluzcoin_fee)
        ...
        tx2 = funding["hex"]
       andaluzcoin_fee2 = Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -(Decimal("10") +andaluzcoin_fee2))
