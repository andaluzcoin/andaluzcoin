// Copyright (c) The Andaluzcoin Core developers
...
fn get_linter_list() -> Vec<&'static Linter> {
    vec![
        ...
        &Linter {
            description: "Check that no symbol from andaluzcoin-build-config.h is used without the header being included",
...
fn lint_includes_build_config() -> LintResult {
    ...
    let print_affected_files = |mode: bool| {
        ...
        let defines_files = check_output(
            git()
                ...
                .args(get_pathspecs_exclude_subtrees())
                .args([
                    // These are exceptions which don't use andaluzcoin-build-config.h, rather CMakeLists.txt adds
    ...
    if missing {
        return Err(format!(
            r#"
One or more files use a symbol declared in the andaluzcoin-build-config.h header. However, they are not
...
even though andaluzcoin-build-config.h indicates that a faster feature is available and should be used.
...
#include <andaluzcoin-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
None of the files use a symbol declared in the andaluzcoin-build-config.h header. However, they are including
